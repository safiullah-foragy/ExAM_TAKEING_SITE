...
router.get('/:id', protect, async (req, res) => {
  try {
    ...
    // Build PDF URL (Direct Supabase Cloud URL or local upload path)
    let pdfUrl = exam.pdfPath;
    if (exam.pdfPath && !exam.pdfPath.startsWith('http')) {
      const pdfRelative = path.relative(path.join(__dirname, '..'), exam.pdfPath);
      pdfUrl = `/${pdfRelative.replace(/\\/g, '/')}`;
    }
